 ...
 * 29-Aug-2002 : Modified to give user control over whether or not
 *               duplicate x-values are allowed (DG);
 * 07-Oct-2002 : Fixed errors reported by Checkstyle (DG);
 * 11-Nov-2002 : Added maximum item count, code contributed by
 *               Jonathan Gabbai (DG);
 * 26-Mar-2003 : Implemented Serializable (DG);
 * 04-Aug-2003 : Added getItems() method (DG);
 * 15-Aug-2003 : Changed 'data' from private to protected, added new
 ...
 *               setMaximumItemCount() method to remove items (and
 *               notify listeners) if necessary, fixed the add() and
 *               addOrUpdate() methods to handle unsortedseries (DG);
 * ------------- JFreeChart 1.0.x ------------------------------------
 * 11-Jan-2005 : Renamed update(int, Number) --> updateByIndex() (DG);
 * 15-Jan-2007 : Added toArray() method (DG);
 * 20-Jun-2007 : Removed deprecated code and JCommon dependencies
 *               (DG);
...
/**
 * Represents a sequence of zero or more data items in the form
 * (x, y).  By default, items in the series will be sorted into
 * ascending order by x-value, and duplicate x-values are permitted.
 * Both the sorting and duplicate defaults can be changed in the
 * constructor.  Y-values can be <code>null</code> to represent
 * missing values.
 */
public class XYSeries extends Series implements Cloneable, Serializable {
    ...
    /**
     * A flag that controls whether the items are automatically
     * sorted.
     */
    private boolean autoSort;
    
    /**
     * A flag that controls whether or not duplicate x-values are
     * allowed.
     */
    ...
    /**
     * Creates a new empty series.  By default, items added to the
     * series will be sorted into ascending order by x-value, and
     * duplicate x-values will be allowed (these defaults can be
    ...
    /**
     * Constructs a new xy-series that contains no data.  You can
     * specify whether or not duplicate x-values are allowed for the
     * series.
     *
     * @param key  the series key (<code>null</code> not permitted).
     * @param autoSort  a flag that controls whether or not the items
     *                  in the series are sorted.
     * @param allowDuplicateXValues  a flag that controls whether
    ...
    /**
     * Returns the flag that controls whether the items in the series
     * are automatically sorted.  There is no setter for this flag, it
    ...
    /**
     * Returns a flag that controls whether duplicate x-values are
     * allowed. This flag can only be set in the constructor.
     *
     * @return A boolean.
     */
    ...
    /**
     * Returns the list of data items for the series (the list
     * contains {@link XYDataItem} objects and is unmodifiable).
     *
    ...
    /**
     * Returns the maximum number of items that will be retained in
    ...
    /**
     * Sets the maximum number of items that will be retained in the
     * series. If you add a new item to the series such that the
     * number of items will exceed the maximum item count, then the
     * first element in the series is automatically removed, ensuring
     * that the maximum item count is not exceeded.
     * <p>
     * Typically this value is set before the series is populated with
     * data, but if it is applied later, it may cause some items to be
     * removed from the series (in which case a
     * {@link SeriesChangeEvent} will be sent to all registered
     * listeners.
     *
     ...
     * unusual pairing of parameter types is to make it easier to add
    ...
    /**
     * Adds a new data item to the series (in the correct position if
     * the <code>autoSort</code> flag is set for the series) and sends
     * a {@link SeriesChangeEvent} to all registered listeners.
     * <P>
