 ...
 * 19-Dec-2001 : Added a new constructor as suggested by Paul English
 *               (DG);
 * 29-Jan-2002 : Worked on the parseWeek() method (DG);
 * 13-Feb-2002 : Fixed bug in Week(Date) constructor (DG);
 * 26-Feb-2002 : Changed getStart(), getMiddle() and getEnd() methods
 *               to evaluate with reference to a particular time zone
 *               (DG);
 ...
 *               consistent with GregorianCalendar. Thanks to
 *               Aimin Han for the code (DG);
 * 02-Jan-2003 : Removed debug code (DG);
 * 13-Mar-2003 : Moved to com.jrefinery.data.time package, and
 ...
 * 30-Sep-2004 : Replaced getTime().getTime() with getTimeInMillis()
 *               (DG);
 * 04-Nov-2004 : Reverted change of 30-Sep-2004, because it won't work
 *               for JDK 1.3 (DG);
 * ------------- JFREECHART 1.0.x ------------------------------------
 * 06-Mar-2006 : Fix for bug 1448828, incorrect calculation of week
 *               and year for the first few days of some years (DG);
 * 05-Oct-2006 : Updated API docs (DG);
 * 06-Oct-2006 : Refactored to cache first and last millisecond values
 *               (DG);
 * 09-Jan-2007 : Fixed bug in next() (DG);
 * 28-Aug-2007 : Added new constructor to avoid problem in creating
 *               new instances (DG);
...
/**
 * A calendar week.  All years are considered to have 53 weeks,
 * numbered from 1 to 53, although in many cases the 53rd week is
 * empty.  Most of the time, the 1st week of the year *begins* in the
 * previous calendar year, but it always finishes in the current year
...
public class Week extends RegularTimePeriod implements Serializable {
    ...
    /**
     * Creates a new time period for the week in which the current
    ...
    /**
     * Creates a time period representing the week in the specified
     * year.
     *
    ...
    /**
     * Creates a time period for the week in which the specified
     * date/time falls.
     *
    ...
    /**
     * Creates a time period for the week in which the specified
     * date/time falls, calculated relative to the specified time
     * zone.
     *
    ...
    /**
     * Returns the first millisecond of the week.  This will be
     * determined relative to the time zone specified in the
     * constructor, or in the calendar instance passed in the most
    ...
    /**
     * Recalculates the start date/time and end date/time for this
     ...
     * <code>null</code> for some lower limit on the range of weeks
     * (currently week 1, 1900).  For week 1 of any year, the previous
     * week is always week 53, but week 53 may not contain any days
     ...
     * <code>null</code> for some upper limit on the range of weeks
     * (currently week 53, 9999).  For week 52 of any year, the
     * following week is always week 53, but week 53 may not contain
    ...
    /**
     * Returns the first millisecond of the week, evaluated using the
    ...
    /**
     * Returns the last millisecond of the week, evaluated using the
     * supplied calendar (which determines the time zone).
     *
     * @param calendar  the calendar (<code>null</code> not
     *                  permitted).
     ...
     * Returns true if the target is a Week instance representing the
     * same week as this object.  In all other cases, returns false.
     *
     * @param obj  the object (<code>null</code> permitted).
     *
     * @return <code>true</code> if week and year of this and object
     *         are the same.
     */
    ...
    /**
     * Returns a hash code for this object instance.  The approach
     * described by Joshua Bloch in "Effective Java" has been used
     * here:
     * <p>
     * <code>
    ...
    /**
     * Returns an integer indicating the order of this Week object
     ...
     * <P>
     * This method is required to accept the format "YYYY-Wnn".  It
     * will also accept "Wnn-YYYY". Anything else, at the moment, is a
     * bonus.
     *
     * @param s  string to parse.
     *
     * @return <code>null</code> if the string is not parseable, the
     *         week otherwise.
     */
     ...
     *
     * @return <code>-1</code> if none of the characters was found,
    ...
    /**
     * Creates a year from a string, or returns null (format
     ...
     *
     * @param s  the string to parse.
     * @return <code>-1</code> if the string does not contain a week
